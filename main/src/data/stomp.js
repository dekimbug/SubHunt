//Global Variables
var AIDM_NAMESPACE="\"http://aidm.aba.navair.navy.mil/\"";
...
(function() {
  ...
  Frame = (function() {
    ...
    Frame.unmarshall = function(datas, isAidm) {
      var data;
      var data_holder;
      var sub_id;
      return (function() {
	...
	_filtered_results = [];
        for (_i = 0, _len = _ref.length; _i < _len; _i++) {
	     ...
	     data_holder = Frame._unmarshallSingle(data);
	     //Capture sub-id
	     if (data_holder.headers.subscription != null) {
		sub_id = data_holder.headers.subscription;
	     }
	     if (isAidm) { //Special processing for frames containing AIDM message
		if (data_holder.body.includes(AIDM_NAMESPACE)){
			//One of the discarded frames included command identifier and sub-id
			data_holder.command = "MESSAGE";
			data_holder.headers.subscription = sub_id;
			//Remove unknown characters from message body
			data_holder.body = data_holder.body.replace(/[^ -~]+/g, "");
            		_results.push(data_holder);
		}
		else {
			console.log("Discarding unneccessary header content captured in message body" + data_holder);		
		}
	     } else { //Processing for all other frames
		_results.push(data_holder);
	     }
          }
        }
  ...
  Client = (function() {
	...
	//Check if message from broker (string) includes AIDM content
	var aidmFlag = data.includes(AIDM_NAMESPACE);
        _ref = Frame.unmarshall(data, aidmFlag);
        _results = [];
        for (_i = 0, _len = _ref.length; _i < _len; _i++) {
          frame = _ref[_i];
	...
	console.log(typeof _results);
	console.log(_results);
	console.log(_results.toString());
        return _results;
      };
