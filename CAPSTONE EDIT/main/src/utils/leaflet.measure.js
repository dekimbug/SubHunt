L.Control.Measure = L.Control.extend({
    ...
    _mouseMove: function (e) {
      ...
      if (this._tooltip) {
        ...
        var bearing = geomath.rhumbBearing([this._lastPoint.lat,this._lastPoint.lng],[e.latlng.lat,e.latlng.lng])
        this._updateTooltipDistance(this._distance + distance, distance,(3600+bearing)%360)
      }
    },
    ...
    _updateTooltipDistance: function (total, difference, bearing) {
      ...
      var roundBearing = Math.round(bearing);
  
      var text = '<div class="leaflet-measure-tooltip-total" style="color:'+ this.options.textColor +'">' + roundBearing + "T/" + totalRound + '</div>'
      if (differenceRound > 0 && totalRound !== differenceRound) {
        text += '<div class="leaflet-measure-tooltip-difference" style="color:'+ this.options.textColor +'">(+' + roundBearing + "T/"  + differenceRound + ')</div>'
      }
    ...
    _formatDistance: function (val) {
      ...
      let yards = val * 1.09361;
      
      if (yards < 4000) {
        return Math.round(yards) + 'yds'
      } else {
        return Math.round((yards * .000493737) * 100) / 100 + 'nm'
      }
    },
